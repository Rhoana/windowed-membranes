...
class ConvNetClassifier(object):
    ...
    def run(self):
        ...
        data                    = preProcess.run()
        train_set_x_np,train_set_y_np = data[0],data[3]
        valid_set_x_np,valid_set_y_np = data[1],data[4]
        test_set_x_np,test_set_y_np   = data[2],data[5]

        print 'Initializing neural network ...'

        # print error if batch size is to large
        if GPU_SIZE<batch_size:
            print 'Error: Batch size is larger than size of validation set.'

        # compute batch sizes for train/test/validation
        n_train_batches  = GPU_SIZE
        n_test_batches   = GPU_SIZE
        n_valid_batches  = GPU_SIZE

        valid_set_x = theano.shared(valid_set_x_np[:GPU_SIZE], borrow=True)
        valid_set_y = theano.shared(valid_set_y_np[:GPU_SIZE], borrow=True)
        ...
        perm = theano.shared(np.arange(train_set_x_np.shape[0]))
        #perm               = theano.shared(np.random.permutation(np.arange(train_set_x.eval().shape[0])))
        rand = theano.shared(np.arange(train_set_x_np.shape[0]))
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                for i in range(train_set_x_np.shape[0] / GPU_SIZE):
                	train_set_x,train_set_y,train_model = f.flip_rotate(train_set_x_np,train_set_y_np,in_window_shape,out_window_shape,perm,index,cost,updates,batch_size,x,y,classifier, GPU=True)
        ...
        for i in range( (test_set_x_np.shape[0] / GPU_SIZE) - 1):
        	test_set_x = theano.shared(train_set_x_np[i*GPU_SIZE:(i+1)*GPU_SIZE], borrow=True)
        	test_set_y = theano.shared(train_set_y_np[i*GPU_SIZE:(i+1)*GPU_SIZE], borrow=True)
